...
local function offer_parser_install()
  ...
  if ft == '' or vim.bo.buftype ~= '' or no_parser_ft[ft] then
    return
  end
  local lang = vim.treesitter.language.get_lang(ft) or ft
  if vim.tbl_contains(settings.treesitter_languages, lang) or declined_parsers[lang] or vim.tbl_contains(require('nvim-treesitter').get_installed(), lang) then
    return
  end
